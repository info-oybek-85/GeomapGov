...
from reports.models import Report, ReportRead, ReportAcceptance, ReportAssignment, ReportRedirect, ReportRejection, ReportWorkEvidence
...
def custom_404_view(request, exception=None):
    ...
    if getattr(request.user, "user_type", None) == UserChoices.EXECUTOR:
...
def org_report_detail(request, pk):
    ...
    if not membership:
        return HttpResponseForbidden("Siz tashkilot a’zosi emassiz")

    org = membership.organization
    is_org_admin = membership.role == OrganizationMember.ROLE_ADMIN or request.user.is_superuser

    report = get_object_or_404(
        Report.objects.select_related("user", "organization").prefetch_related("attachments", "work_evidence"),
        pk=pk,
        organization=org
    )
    ...
    can_act = is_org_admin and report.status in (ReportStatus.NEW, ReportStatus.SENT, ReportStatus.READ)
    ...
    if request.method == "POST":
        ...
        if action == "start_work":
            assignment = ReportAssignment.objects.filter(
                report=report, organization=org, assigned_to=request.user
            ).order_by("-assigned_at").first()
            if not assignment and not request.user.is_superuser:
                messages.error(request, "Bu murojaat sizga biriktirilmagan.")
            ...
            messages.success(request, "Ishni boshlash vaqti qayd etildi.")
            return redirect(request.path)

        if action == "complete_work":
            note = (request.POST.get("completion_note") or "").strip()
            if len(note) < 10:
                messages.error(request, "Bajarilgan ish haqida kamida 10 ta belgi yozing.")
                return redirect(request.path)
            assignment = ReportAssignment.objects.filter(
                report=report, organization=org, assigned_to=request.user
            ).order_by("-assigned_at").first()
            ...
            if not evidence_files and not report.work_evidence.exists():
                messages.error(request, "Bajarilgan ishni tasdiqlovchi kamida bitta foto yoki fayl yuklang.")
                return redirect(request.path)
            for uploaded in evidence_files:
                if uploaded.size > 15 * 1024 * 1024:
                    messages.error(request, f"{uploaded.name}: fayl hajmi 15 MB dan oshmasligi kerak.")
                    return redirect(request.path)
                ReportWorkEvidence.objects.create(
                    report=report, assignment=assignment, uploaded_by=request.user,
                    file=uploaded, original_name=uploaded.name,
                    mime_type=getattr(uploaded, "content_type", "") or "", file_size=uploaded.size,
                )
            ...
            report.save(update_fields=["work_started_at", "work_completed_at", "completion_note", "status", "updated_at"])
            if getattr(report.user, "telegram_id", None):
                send_telegram_message(report.user.telegram_id, "✅ Muammo bo‘yicha ish bajarildi. Tashkilot siz bilan bog‘lanib, natijani tasdiqlashingizni so‘raydi.")
            messages.success(request, "Ish bajarildi. Endi fuqaro tasdig‘i kutilmoqda.")
            return redirect(request.path)

        if action in {"citizen_confirm", "citizen_reject"}:
            # Yakuniy natijani tashkilot xodimi yoki admini belgilamaydi.
            # Tasdiq/rad javobi faqat murojaat egasidan Telegram bot/API orqali olinadi.
            return HttpResponseForbidden(
                "Fuqaro tasdig‘i faqat murojaat egasi tomonidan Telegram bot orqali beriladi."
            )

        if not can_act:
            messages.error(request, "Bu shikoyat bo‘yicha ushbu amalni bajarib bo‘lmaydi.")
            return redirect(request.path)
        ...
        if action == "accept_assign":
            ...
            deadline_hours_raw = request.POST.get("deadline_hours") or "24"
            try:
                deadline_hours = max(1, min(int(deadline_hours_raw), 720))
            except ValueError:
                deadline_hours = 24
            ...
            for m in valid_memberships:
                ReportAssignment.objects.get_or_create(
                    ...
                    organization=org,
                    assigned_to=m.user,
                    defaults={"assigned_by": request.user, "deadline_at": deadline_at}
                )

            # status va vaqtlar
            report.status = ReportStatus.ASSIGNED
            report.accepted_at = report.accepted_at or timezone.now()
            report.assigned_at = report.assigned_at or timezone.now()
            report.deadline_at = deadline_at
            report.save(update_fields=["status", "accepted_at", "assigned_at", "deadline_at", "updated_at"])
            ...
            if getattr(report.user, "telegram_id", None):
                ...
                msg = (
                    ...
                    f"Shikoyat holati: Xodimga biriktirildi"
                )
                send_telegram_message(report.user.telegram_id, msg)

            # Har bir biriktirilgan xodimga Telegram push-xabari
            from organizations.models import TelegramStaffLink
            for membership_item in valid_memberships:
                staff_link = TelegramStaffLink.objects.filter(
                    user=membership_item.user, is_active=True
                ).first()
                if not staff_link:
                    continue
                short_description = (report.description or "")[:500]
                citizen_name = report.user.get_full_name() or report.user.username
                citizen_phone = report.user.phone_number or "Ko‘rsatilmagan"
                staff_message = (
                    f"🔔 Sizga yangi murojaat biriktirildi.\n\n"
                    f"📌 {report.title}\n"
                    f"👤 Kimdan: {citizen_name}\n"
                    f"📞 Telefoni: {citizen_phone}\n"
                    f"🏢 {org.name}\n"
                    f"⏰ Muddat: {deadline_at.strftime('%d.%m.%Y %H:%M')}\n"
                    f"📝 {short_description}"
                )
                staff_buttons = [[{
                    "text": "👷 Vazifani ochish",
                    "callback_data": f"staff:task:{report.id}",
                }]]
                if report.latitude is not None and report.longitude is not None:
                    staff_buttons.append([{
                        "text": "📍 Xaritada ochish",
                        "url": f"https://www.google.com/maps?q={report.latitude},{report.longitude}",
                    }])
                staff_buttons.append([{
                    "text": "▶️ Ishni boshlash",
                    "callback_data": f"staff:start:{report.id}",
                }])
                send_telegram_message(
                    staff_link.telegram_id,
                    staff_message,
                    reply_markup={"inline_keyboard": staff_buttons},
                )

            messages.success(request, "Qabul qilindi va tanlangan xodimlarga biriktirildi.")
            return redirect(request.path)
    ...
    latest_assignment = current_assignments.first()
    is_assigned_worker = current_assignments.filter(assigned_to=request.user).exists()
    can_start_work = is_assigned_worker and report.status in (ReportStatus.ASSIGNED, ReportStatus.REOPENED)
    can_complete_work = is_assigned_worker and report.status in (ReportStatus.IN_PROGRESS, ReportStatus.REOPENED, ReportStatus.ASSIGNED)
    can_confirm_citizen = report.status == ReportStatus.PENDING_CONFIRMATION
    ...
    return render(request, "organization_admin/report_detail.html", {
        ...
        "can_complete_work": can_complete_work,
        "can_confirm_citizen": False,
