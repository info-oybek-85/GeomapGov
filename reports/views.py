...
class ReportResolveView(APIView):
    ...
    def post(self, request, pk):
        ...
        # XAVFSIZLIK QOIDASI: fuqaro faqat tashkilot xodimi ishni
        # yakunlab, murojaatni tasdiqqa yuborganidan keyin tasdiqlashi mumkin.
        if report.status != ReportStatus.PENDING_CONFIRMATION:
            return Response(
                {
                    "detail": (
                        "Murojaat hali fuqaro tasdig‘iga yuborilmagan. "
                        "Uni 'Hal qilindi' deb belgilash mumkin emas."
                    )
                },
        ...
        report.citizen_confirmed_at = now
        report.resolved_at = now
        report.citizen_confirmation_note = "Fuqaro Telegram bot orqali tasdiqladi."
        report.save(
            update_fields=[
                "status",
                "citizen_confirmed_at",
                "resolved_at",
                "citizen_confirmation_note",
                "updated_at",
            ]
        )
