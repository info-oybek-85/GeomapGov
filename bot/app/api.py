...
class ApiClient:
    ...
    async def predict_organization(
        self,
        session: aiohttp.ClientSession,
        access_token: str,
        text: str,
    ) -> Dict[str, Any]:
        url = f"{self.base_url}/organizations/predict/"
        headers = {"Authorization": f"Bearer {access_token}"}
        async with session.post(url, json={"text": text}, headers=headers) as resp:
            ...
            if resp.status == 401:
                raise ApiError("UNAUTHORIZED")
            if resp.status != 200:
                raise ApiError(f"Organization prediction error: {resp.status} {str(data)[:500]}")
            return data

    async def save_organization_prediction_feedback(
        self,
        session: aiohttp.ClientSession,
        access_token: str,
        payload: Dict[str, Any],
    ) -> Dict[str, Any]:
        url = f"{self.base_url}/organizations/prediction-feedback/"
        headers = {"Authorization": f"Bearer {access_token}"}
        async with session.post(url, json=payload, headers=headers) as resp:
            ...
            if resp.status not in (200, 201):
                raise ApiError(f"Prediction feedback error: {resp.status} {str(data)[:500]}")
            return data
    ...
    async def staff_login(self, session, telegram_id: int, username: str, password: str):
        url = f"{self.base_url}/staff/telegram-login/"
        async with session.post(url, json={"telegram_id": telegram_id, "username": username, "password": password}) as resp:
            data = await resp.json(content_type=None)
            if resp.status != 200:
                raise ApiError(data.get("detail") or f"Staff login error: {resp.status}")
            return data

    async def staff_tasks(self, session, access_token: str, period: str = "all", status: str = "all"):
        url = f"{self.base_url}/staff/tasks/?period={period}&status={status}"
        async with session.get(url, headers={"Authorization": f"Bearer {access_token}"}) as resp:
            data = await resp.json(content_type=None)
            if resp.status == 401:
                raise ApiError("UNAUTHORIZED")
            if resp.status != 200:
                raise ApiError(data.get("detail") or f"Staff tasks error: {resp.status}")
            return data

    async def staff_task_detail(self, session, access_token: str, report_id: str):
        url = f"{self.base_url}/staff/tasks/{report_id}/"
        async with session.get(url, headers={"Authorization": f"Bearer {access_token}"}) as resp:
            data = await resp.json(content_type=None)
            if resp.status == 401:
                raise ApiError("UNAUTHORIZED")
            if resp.status != 200:
                raise ApiError(data.get("detail") or f"Staff task error: {resp.status}")
            return data

    async def staff_start_task(self, session, access_token: str, report_id: str):
        url = f"{self.base_url}/staff/tasks/{report_id}/start/"
        async with session.post(url, headers={"Authorization": f"Bearer {access_token}"}) as resp:
            data = await resp.json(content_type=None)
            if resp.status != 200:
                raise ApiError(data.get("detail") or f"Start task error: {resp.status}")
            return data

    async def staff_complete_task(self, session, access_token: str, report_id: str, note: str, files):
        url = f"{self.base_url}/staff/tasks/{report_id}/complete/"
        form = aiohttp.FormData()
        form.add_field("completion_note", note)
        for filename, content, content_type in files:
            form.add_field("evidence_files", content, filename=filename, content_type=content_type)
        async with session.post(url, data=form, headers={"Authorization": f"Bearer {access_token}"}) as resp:
            data = await resp.json(content_type=None)
            if resp.status != 200:
                raise ApiError(data.get("detail") or f"Complete task error: {resp.status}")
            return data

    async def staff_map_link(self, session, access_token: str):
        url = f"{self.base_url}/staff/map-link/"
        async with session.get(url, headers={"Authorization": f"Bearer {access_token}"}) as resp:
            data = await resp.json(content_type=None)
            if resp.status != 200:
                raise ApiError(data.get("detail") or f"Map link error: {resp.status}")
            return data
