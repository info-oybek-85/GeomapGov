...
from ..keyboards import (
    organizations_kb, OrgCb, OrgSuggestionCb, organization_suggestion_kb,
)
...
async def _show_report_preview(
    message: Message,
    state: FSMContext,
    organization_id: str,
    ...
    await state.update_data(
        ...
        selected_org_name=organization_name,
    )
...
async def _accept_location(message: Message, state: FSMContext, db: BotDB, api: ApiClient, lat: float, lon: float):
    ...
    if user and description:
        async def predict(access_token: str):
            async with aiohttp.ClientSession() as session:
                return await api.predict_organization(session, access_token, description)

        try:
            prediction = await predict(user["access_token"])
        except ApiError as e:
            if str(e) == "UNAUTHORIZED":
                new_access = await _refresh_tokens(db, api, telegram_id, user)
                prediction = await predict(new_access)
            else:
                prediction = {}

        organization = prediction.get("organization") or {}
        ...
        if organization.get("id") and confidence >= 0.48:
            await state.update_data(
                predicted_organization_id=str(organization["id"]),
                predicted_organization_name=organization.get("name") or "Tashkilot",
                organization_prediction_confidence=confidence,
                organization_recommendations=recommendations,
                organization_matched_keywords=matched_keywords,
            )
            lines = ["🤖 <b>Murojaat matni tahlil qilindi.</b>", "", "TOP-3 tavsiya:"]
            for index, item in enumerate(recommendations[:3], start=1):
                pct = round(float(item.get("confidence") or 0) * 100)
                lines.append(f"{index}. <b>{html.escape(str(item.get('name') or 'Tashkilot'))}</b> — {pct}%")
            if matched_keywords:
                words = ", ".join(html.escape(str(x)) for x in matched_keywords[:6])
                lines.extend(["", f"🔎 Aniqlangan iboralar: <i>{words}</i>"] )
            elif explanations:
                lines.extend(["", f"ℹ️ {html.escape(str(explanations[0]))}"] )
            lines.extend(["", "Tegishli tashkilotni tanlang yoki boshqa tashkilotni oching."])
            await message.answer(
                "\n".join(lines),
                reply_markup=organization_suggestion_kb(recommendations),
                parse_mode="HTML",
            )
            return

    await message.answer(
        "🤷 Tizim tashkilotni yetarli ishonch bilan aniqlay olmadi. Tashkilotni tanlang:",
    )
    await _load_org_page(message, state, db, api, page=1, telegram_id=telegram_id)
...
@router.callback_query(ReportCreate.waiting_organization, OrgSuggestionCb.filter())
async def organization_suggestion_handler(
    call: CallbackQuery,
    callback_data: OrgSuggestionCb,
    state: FSMContext,
    db: BotDB,
    api: ApiClient,
):
    ...
    if callback_data.action == "other":
        await call.answer("Tashkilotlar ro‘yxati")
        await _load_org_page(msg, state, db, api, page=1, telegram_id=telegram_id)
        return

    if callback_data.action == "pick":
        data = await state.get_data()
        org_id = callback_data.org_id
        recommendations = data.get("organization_recommendations") or []
        selected = next((x for x in recommendations if str(x.get("id")) == str(org_id)), None)
        org_name = (selected or {}).get("name") or "Tashkilot"
        if not org_id:
            await call.answer("Tavsiya topilmadi", show_alert=True)
            await _load_org_page(msg, state, db, api, page=1, telegram_id=telegram_id)
            return
        await state.update_data(prediction_choice_source="top3")
        await call.answer("✅ Tashkilot tanlandi")
        await _show_report_preview(msg, state, str(org_id), str(org_name))
        return
...
async def org_pick_or_page(call: CallbackQuery, callback_data: OrgCb, state: FSMContext, db: BotDB, api: ApiClient):
    ...
    if action == "pick":
        ...
        await state.update_data(organization_id=org_id, prediction_choice_source="manual")
        await call.answer("✅ Tashkilot tanlandi")
        ...
        await state.update_data(selected_org_name=org_name or str(org_id))
...
async def report_submit_confirmed(message: Message, state: FSMContext, db: BotDB, api: ApiClient):
    ...
    # ML tavsiyasi yoki qo‘lda tanlangan tashkilot nomi
    org_name = data.get("selected_org_name")
    if not org_name:
    ...
    # ML tavsiyasi va foydalanuvchi yakuniy tanlovini self-learning dataset uchun saqlaymiz.
    predicted_org_id = data.get("predicted_organization_id")
    if predicted_org_id:
        feedback_payload = {
            "text": description,
            "predicted_organization_id": predicted_org_id,
            "selected_organization_id": str(organization_id),
            "confidence": float(data.get("organization_prediction_confidence") or 0.0),
            "recommendations": data.get("organization_recommendations") or [],
            "matched_keywords": data.get("organization_matched_keywords") or [],
            "report_id": created.get("id"),
        }
        async def save_feedback(access_token: str):
            async with aiohttp.ClientSession() as session:
                return await api.save_organization_prediction_feedback(session, access_token, feedback_payload)
        try:
            await save_feedback(user["access_token"])
        except ApiError as e:
            if str(e) == "UNAUTHORIZED":
                try:
                    new_access = await _refresh_tokens(db, api, telegram_id, user)
                    await save_feedback(new_access)
                except Exception:
                    pass
            # Feedback yozilmasa murojaat yuborish muvaffaqiyatiga xalaqit bermaydi.
