...
async def show_current_report(message_or_query, state: FSMContext):
    ...
    # Fuqaro tasdig‘i tugmasi faqat tashkilot ishni yakunlab,
    # murojaatni pending_confirmation holatiga o‘tkazganda chiqadi.
    can_resolve = (
        not resolved_list
        and str(r.get("status") or "").lower() == "pending_confirmation"
    )
