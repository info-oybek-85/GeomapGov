...
class OrganizationPredictionView(APIView):
    permission_classes = [IsAuthenticated]

    def post(self, request):
        from .organization_classifier import predict_organization

        text = (request.data.get("text") or "").strip()
        if len(text) < 5:
            return Response(
                {"detail": "Murojaat matni kamida 5 ta belgidan iborat bo‘lishi kerak."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        result = predict_organization(text)
        return Response(result, status=status.HTTP_200_OK)


class OrganizationPredictionFeedbackView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request):
        ...
        text = (request.data.get("text") or "").strip()
        predicted_id = request.data.get("predicted_organization_id")
        selected_id = request.data.get("selected_organization_id")
        report_id = request.data.get("report_id")
        confidence = request.data.get("confidence") or 0.0
        recommendations = request.data.get("recommendations") or []
        matched_keywords = request.data.get("matched_keywords") or []

        if len(text) < 5 or not selected_id:
            return Response(
                {"detail": "Matn va tanlangan tashkilot majburiy."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        predicted = Organization.objects.filter(id=predicted_id).first() if predicted_id else None
        selected = Organization.objects.filter(id=selected_id).first()
        if not selected:
            return Response({"detail": "Tanlangan tashkilot topilmadi."}, status=404)

        report = Report.objects.filter(id=report_id, user=request.user).first() if report_id else None
        feedback = OrganizationPredictionFeedback.objects.create(
            user=request.user,
            report=report,
            text=text,
            predicted_organization=predicted,
            selected_organization=selected,
            confidence=float(confidence),
            accepted=bool(predicted and predicted.id == selected.id),
            recommendations=recommendations if isinstance(recommendations, list) else [],
            matched_keywords=matched_keywords if isinstance(matched_keywords, list) else [],
        )
        return Response({"id": feedback.id, "accepted": feedback.accepted}, status=201)
